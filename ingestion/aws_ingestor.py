"""
AWS CUR ingestor — Layer 1 (Raw ingestion only).

Reads raw AWS CUR CSVs, validates that required columns exist,
and writes Parquet as-is to data/raw/aws/YYYY-MM/.
No transformations — all casting, parsing, and derivations happen in dbt staging.
"""
import logging
...
from ingestion.schemas.aws_cur_schema import REQUIRED_COLUMNS, DATE_PARTITION_COLUMN

logger = logging.getLogger(__name__)

REPO_ROOT  = Path(__file__).parent.parent
SYNTHETIC  = REPO_ROOT / "data" / "synthetic" / "aws"
RAW_OUTPUT = REPO_ROOT / "data" / "raw" / "aws"
...
def _billing_month(df: pd.DataFrame) -> str:
    """Extract YYYY-MM from the date partition column (string, no parsing)."""
...
def ingest(source_dir: Path = SYNTHETIC, output_dir: Path = RAW_OUTPUT) -> dict:
    ...
    for csv_path in csv_files:
        ...
        # Load as strings — no coercions, preserve raw values exactly
...
if __name__ == "__main__":
    ...
    summary = ingest()
    print(f"\nAWS ingestion complete — {sum(summary.values())} rows written.")
