...
select
    md5(concat_ws('|', r.entity_id, r.recommendation_id, 'finops_decision')) as decision_id,
    r.entity_id,
    ...
    case
        when a.candidate_count > 1 then
            concat(
                ...
                ' because ', r.root_cause_type,
                ' scored highest under the FinOps priority model.'
            )
        else
            concat(
                'Selected ', r.action_type,
                ' because it is the only active FinOps recommendation candidate for this entity.'
            )
