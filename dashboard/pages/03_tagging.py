...
st.caption("Governance control layer — coverage analytics, ownership assignment, and SLA tracking.")
...
# ---------------------------------------------------------------------------
# Helpers — load/save owner assignments
# ---------------------------------------------------------------------------
...
view = st.radio(
    "View",
    ["Raw (tagged rows only)", "Adjusted (after heuristic allocation)"],
    horizontal=True,
    help=(
        "**Raw**: counts only rows with an explicit `tag_team` value.  \n"
        "**Adjusted**: includes heuristic allocation — shows how much of the gap can be "
        "closed programmatically. Remaining gap requires tag enforcement."
    ),
)
...
is_attributable = (
    df["allocated_team"].fillna("unattributed") != "unattributed"
    if adjusted else is_tagged
)

total_nec        = df["nec"].sum()
attributed_nec   = df.loc[is_attributable, "nec"].sum()
unattributed_nec = total_nec - attributed_nec
attributed_pct   = attributed_nec / total_nec * 100 if total_nec else 0
...
c1.metric("Attributed NEC",      f"{attributed_pct:.1f}%",
          delta=f"{unattributed_pct:.1f}% unattributed", delta_color="inverse")
c2.metric("Attributed $",        f"${attributed_nec:,.0f}")
c3.metric("Unattributed $",      f"${unattributed_nec:,.0f}", delta_color="inverse")
c4.metric("Owners Assigned",     str(len(_assigned_map)),
          delta=f"{len(_assigned_map)} accounts mapped")

if adjusted:
    raw_pct = is_tagged.mean() * 100
    gain    = attributed_pct - raw_pct
    if gain > 0.5:
        st.success(
            f":white_check_mark: **Heuristic allocation recovers +{gain:.1f}%** — "
            f"{raw_pct:.1f}% (explicit tags) → **{attributed_pct:.1f}%** (after allocation). "
            "Remaining gap requires policy enforcement."
        )

if unattributed_pct >= 20:
    st.error(
        f":red_circle: **HIGH — {unattributed_pct:.1f}% of NEC (${unattributed_nec:,.0f}) unattributed.** "
        "Use the Assign Owner section below to close the gap. Target: <5% within 60 days."
    )
elif unattributed_pct >= 10:
    st.warning(
        f":orange_circle: **MEDIUM — {unattributed_pct:.1f}% (${unattributed_nec:,.0f}) unattributed.** "
        "See Assign Owner section below."
    )
else:
    st.success(f":green_circle: **Coverage acceptable** — {attributed_pct:.1f}% attributed.")

st.markdown("---")

# ---------------------------------------------------------------------------
# Section 1 — Tag Policy Compliance (Required vs Optional)
# ---------------------------------------------------------------------------
...
# Coverage for `team` tag — real data
team_tag_cov = is_tagged.mean() * 100

# Simulate environment + cost_center coverage (correlated with tagging state)
# In a real pipeline these would be separate columns; here we derive from billing patterns.
...
st.dataframe(req_df.style.map(_type_color, subset=["Type"]),
             use_container_width=True, hide_index=True)

# Tag quality score (0–5)
...
quality_color = COLOR_OPTIMIZED if quality_score >= 4 else (COLOR_INEFFICIENCY if quality_score >= 2.5 else COLOR_BLOCKER)
...
df_view  = df.assign(is_attr=is_attributable)
by_cloud = (
    df_view.groupby("cloud_provider")
    .apply(lambda g: pd.Series({
        "attributed_pct": (g.loc[g["is_attr"], "nec"].sum() / g["nec"].sum() * 100)
                          if g["nec"].sum() else 0.0,
    }), include_groups=False)
    .reset_index()
)
fig_cloud = px.bar(
    by_cloud, x="cloud_provider", y="attributed_pct",
    ...
    range_y=[0, 115], text="attributed_pct",
)
fig_cloud.update_traces(texttemplate="%{text:.1f}%", textposition="outside")
fig_cloud.add_hline(y=95, line_dash="dot", line_color="#22c55e",
...
fig_svc = px.bar(
    by_svc, x="attributed_pct", y="service_category", orientation="h",
    labels={"attributed_pct": "Attributed NEC %", "service_category": "Service Category"},
    color="attributed_pct", color_continuous_scale="RdYlGn", range_color=[0, 100],
    text="attributed_pct",
)
...
# ---------------------------------------------------------------------------
# Section 3 — Assign Owner Now (actionable control)
# ---------------------------------------------------------------------------

st.subheader("Assign Owner Now")
st.caption(
    "Assign a team to each unattributed account. Assignments are persisted locally and "
    "surfaced in the SLA tracker below. Apply to your provisioning templates to make it permanent."
)
...
if unattr_df.empty:
    ...
    if not by_acct.empty and by_acct.iloc[0]["pct_of_total"] >= 15:
        w = by_acct.iloc[0]
        currently_assigned = _assigned_map.get(w["account_id"])
        assign_status = f" (currently → **{currently_assigned}**)" if currently_assigned else " (unassigned)"
        st.error(
            f":dart: **Top gap: `{w['account_id']}` ({w['cloud_provider'].upper()})** — "
            f"${w['unattr_nec']:,.0f} ({w['pct_of_total']:.0f}% of all unattributed NEC){assign_status}.  \n"
            "Assigning an owner here closes the largest share of the attribution gap in one action."
        )
    ...
    with st.form("assign_owner_form"):
        for _, row in top_accounts.iterrows():
            ...
            current   = _assigned_map.get(acct_id, "")
            ...
            with col_select:
                ...
                chosen = st.selectbox(
                    ...
                    team_opts,
                    index=default_idx,
                    key=f"assign_{acct_id}",
                    label_visibility="collapsed",
                )
                if chosen != "— unassigned —":
                    new_assignments[acct_id] = chosen

        submitted = st.form_submit_button("Apply Assignments", type="primary")
        if submitted and new_assignments:
            existing = {a["account_id"]: a for a in assignments_store.get("assignments", [])}
            for acct_id, team in new_assignments.items():
                cloud_val = by_acct.loc[by_acct["account_id"] == acct_id, "cloud_provider"].iloc[0] if acct_id in by_acct["account_id"].values else "unknown"
                existing[acct_id] = {
                    "account_id":     acct_id,
            ...
            _assigned_map.update(new_assignments)
            st.success(f"Assigned {len(new_assignments)} account(s). Refresh to see SLA tracker update.")
...
if unattr_df.empty:
    ...
    # First billing_month per unattributed account as proxy for "first detected"
    unattr_first = (
        unattr_df.groupby(["cloud_provider", "account_id"])["billing_month"]
        .min().reset_index(name="first_seen_month")
    )
    ...
    def _sla_bucket(days: int) -> str:
        if days <= 3:   return "New (0–3 days)"
        if days <= 7:   return "Warning (4–7 days)"
    ...
    acct_nec = (
        ...
        .sum().reset_index(name="unattr_nec")
    )
    sla_df = unattr_first.merge(acct_nec, on=["cloud_provider", "account_id"], how="left")
    sla_df["Assigned"] = sla_df["account_id"].map(lambda a: _assigned_map.get(a, "Unassigned"))
    ...
    st.dataframe(
        sla_display.style
            .map(_sla_row_color, subset=["SLA Status"])
            .map(_assigned_color, subset=["Assigned To"]),
        use_container_width=True, hide_index=True,
    )
    ...
    if not escalations.empty:
        ...
        st.error(
            f":rotating_light: **{len(escalations)} account(s) past 7-day SLA** — "
...
# ---------------------------------------------------------------------------
# Section 5 — Ownership Gap (existing, collapsed for brevity)
# ---------------------------------------------------------------------------
...
if not unattr_df.empty:
    ...
    if not _worst.empty and _worst.iloc[0]["unattr_nec"] > 0:
        w = _worst.iloc[0]
        w_pct = w["unattr_nec"] / unattributed_nec * 100 if unattributed_nec else 0
        currently = _assigned_map.get(w["account_id"])
        status_str = f"Assigned → **{currently}**" if currently else "**Not yet assigned**"
        if w_pct >= 15:
            st.error(
                f":dart: **Single biggest gap: `{w['account_id']}` ({w['cloud_provider'].upper()})** — "
                f"${w['unattr_nec']:,.0f} ({w_pct:.0f}% of all unattributed NEC). {status_str}"
            )

if not unattr_df.empty:
    with st.expander("Full unattributed account table"):
        disp = by_acct[["Priority", "cloud_provider", "account_id",
                        "unattr_rows", "unattr_nec", "unattr_pct"]].copy()
        ...
        st.dataframe(disp.style.map(_risk_color, subset=["Priority"]),
                     use_container_width=True, hide_index=True)

# ---------------------------------------------------------------------------
# Section 6 — Secondary details (collapsed)
# ---------------------------------------------------------------------------
...
with st.expander("Unattributed NEC by service (top 20)"):
    if not unattr_df.empty:
        ...
        fig_s = px.bar(
            by_svc2, x="unattr_nec", y="service_name", color="cloud_provider",
            orientation="h",
            color_discrete_map={"aws": "#f97316", "azure": "#2563eb", "gcp": "#22c55e"},
            labels={"unattr_nec": "Unattributed NEC (USD)", "service_name": "Service",
                    "cloud_provider": "Cloud"},
            text="pct",
        )
        fig_s.update_traces(texttemplate="%{text:.1f}%", textposition="outside")
        fig_s.update_layout(height=420, margin=dict(t=10, b=40),
...
if "usage_date" in df.columns:
    with st.expander("Attribution trend over time"):
        trend = (
            ...
            .apply(lambda g: pd.Series({"unattr_nec": g.loc[~g["is_attr"], "nec"].sum()}),
                   include_groups=False)
            .reset_index()
        )
        ...
        if not trend.empty:
            fig_t = px.line(
                trend, x="date", y="unattr_nec", color="cloud_provider",
